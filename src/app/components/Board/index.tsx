...
const Board:FC<PropsType> = ({board}) => {
    return(
       ...
       <Droppable droppableId={"droppable-" + board.id }>
        {(provided) => (
            <div className=" w-1/4 rounded-lg border-2 border-slate-950 h-2/3 bg-slate-100   "
                ...
                </div>
                {
                    board?.tasks.map((task, i)=> (
                        <Draggable key={i} draggableId={task.id.toString() } index={i}>
                            {(provided, snapshot)=>(
                                <div className={`card bg-base-100 w-full  shadow-xl my-5 p-5`}
                                ...
                                >
                                    <div className="card-body">
                                        <div className="flex justify-center items-center">
                                            ...
                                            <span className={`badge badge-${status[task.priority]} badge-outline ml-5`}>
                                                {task.priority}
                                            </span>
                                        </div>
